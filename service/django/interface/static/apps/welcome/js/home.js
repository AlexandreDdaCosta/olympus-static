...
$(document).ready(function(){
    ...
    var albumOffset = $('#blog_summary').offset().top - breakOffset;
    var appsOffset = $('#album_summary').offset().top - breakOffset;
    var endOffset = $('#apps_summary').offset().top - breakOffset
    if (pxToTop < blogOffset){
        jQuery('body').removeClass('blog_background');
        jQuery('body').removeClass('album_background');
        jQuery('body').removeClass('apps_background');
    }else if (pxToTop >= blogOffset && pxToTop < albumOffset){
        jQuery('body').addClass('blog_background');
        jQuery('body').removeClass('album_background');
        jQuery('body').removeClass('apps_background');
    }else if (pxToTop >= albumOffset && pxToTop < appsOffset){
        jQuery('body').addClass('album_background');
        jQuery('body').removeClass('blog_background');
        jQuery('body').removeClass('apps_background');
...
.setTween("#album_overlay", .5, {opacity:0, ease: Power1.easeOut})
//.addIndicators({name:'album_overlay_fadeIn',colorStart:'orange',colorTrigger:'purple' })
.addTo(controller);

new ScrollMagic.Scene({
 triggerElement: '#album_summary',
 duration: "50%",
 offset: -100 
})
.setTween("#album_overlay", .5, {opacity:1, ease: Power1.easeOut})
//.addIndicators({name:'album_overlay_fadeOut',colorStart:'purple',colorTrigger:'yellow' })
.addTo(controller);
